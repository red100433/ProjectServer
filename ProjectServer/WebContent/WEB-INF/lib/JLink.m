...
*)
If[StringMatchQ["10.3.1", DigitCharacter ~~ ___] && ToExpression["10.3.1"] == $VersionNumber && !ValueQ[`Private`foundMX],
    Quiet[
        Check[
            `Private`foundMX =
...
JLink`Information`$BuildNumber = If[SyntaxQ["137"], ToExpression["137"], 0]
JLink`Information`$CreationID = If[SyntaxQ["20151227201453"], ToExpression["20151227201453"], 0]
JLink`Information`$CreationDate = If[SyntaxQ["{2015,12,27,20,14,53}"], ToExpression["{2015,12,27,20,14,53}"], {0,0,0,0,0,0}]
JLink`Information`$Version = "J/Link Version 4.9.0"
