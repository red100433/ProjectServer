...
AddToClassPath[locs__String, opts:OptionsPattern[]] := AddToClassPath[GetJVM[InstallJava[]], {locs}, opts]
AddToClassPath[locs:{__String}, opts:OptionsPattern[]] := AddToClassPath[GetJVM[InstallJava[]], locs, opts]
AddToClassPath[jvm_JVM, locs__String, opts:OptionsPattern[]] := AddToClassPath[jvm, {locs}, opts]
AddToClassPath[jvm_JVM, locs:{__String}, OptionsPattern[]] :=
    If[checkJVM[jvm],
		jAddToClassPath[jvm, locs, True, TrueQ[OptionValue[Prepend]]];
...
JavaWindowToFront[id_String] :=
	asyncEvaluate[
		Module[{windowList},
			windowList = Cases[registeredWindows[], {obj_, ToExpression[id], "Java", ___} -> obj];
...
registerWindow[obj_, title_String, type_String, id_Integer, extra___] :=
	If[!windowRegisteredQ[obj],
		...
		AppendTo[$registeredWindows, {obj, id, type, extra}];
		True,
	(* else *)
		(* Already registered. *)
		False
...
unregisterWindow[obj_] :=
	Module[{windowRec, id},
		...
		If[Length[windowRec] > 0,
			...
			$registeredWindows = DeleteCases[$registeredWindows, {obj, id, __}]
		]
	]

(* Type will be "Java" or ".NET". This func is called in UninstallJava/NET. *)
unregisterAllWindows[type_String, extra___] :=
	unregisterWindow /@ First /@ Cases[registeredWindows[], {_, _, type, extra}]
